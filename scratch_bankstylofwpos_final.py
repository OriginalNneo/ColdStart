"""
Generate the Iter-22 prediction (fw + pseudo-POS) on the REAL test (uncapped): add the function-word
...
Writes predictions/Task3_BankStyloFWPOS_Prediction.csv. No submission here.
"""
import re, time
...
SEED = 42
t0 = time.time()
FRAC, ROUNDS, WS, ALPHA, BANK_S, STYLO_S, FW_W, POS_W = 0.5, 1, 1.6, 0.9, 0.02, 0.04, 1.0, 1.0
...
def pos_tag(w, sent_start):
    if w.isdigit(): return "CD"
    if not w[0].isalpha(): return w
    wl = w.lower()
    if wl in _FW: return "FW"
...
def pos_seq(t):
    out, ss = [], True
    for m in _tok.findall(t):
        out.append(pos_tag(m, ss)); ss = m in _END
    return " ".join(out)
...
def main():
    ...
    ps_tr = [pos_seq(t) for t in texts]; ps_te = [pos_seq(t) for t in test_texts]
    pv = TfidfVectorizer(analyzer="word", ngram_range=(1, 3), min_df=3, sublinear_tf=True, token_pattern=r"[^ ]+")
    Ps_tr = pv.fit_transform(ps_tr).astype(np.float32); Ps_te = pv.transform(ps_te).astype(np.float32)

    Ftr = sparse.hstack([Xtr, sparse.csr_matrix(sb.transform(Btr) * BANK_S),
                         sparse.csr_matrix(ss.transform(Styr) * STYLO_S), Fw_tr * FW_W, Ps_tr * POS_W]).tocsr()
    Fte = sparse.hstack([Xte, sparse.csr_matrix(sb.transform(Bte) * BANK_S),
                         sparse.csr_matrix(ss.transform(Stye) * STYLO_S), Fw_te * FW_W, Ps_te * POS_W]).tocsr()
    ...
    out = "predictions/Task3_BankStyloFWPOS_Prediction.csv"
